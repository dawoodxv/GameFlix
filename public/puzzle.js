if (typeof SpaceshipGame === "undefined") {
  class PuzzleGame {
    ...
    preload() {
      let filename = "target" + this.targetImage + ".png";
      this.loadedTargetImage = loadImage(filename, () => {
        this.loadedTargetImage.width = this.GRID_WIDTH;
        this.loadedTargetImage.height = this.GRID_HEIGHT;
      });
      this.setup();
    }
    ...
    setup() {
      ...
      background(0);
      this.targetImage = this.loadedTargetImage;
    }

    draw() {
      // Draw the color wells
      this.drawColourWells(
        this.wellX,
        this.wellY,
        this.NUM_OF_WELLS,
        this.WELL_SIZE,
      ...
      // Draw the coloring grid
      this.drawColouringGrid(
        this.NUM_OF_COLS,
        this.NUM_OF_ROWS,
        this.CELL_SIZE,
        this.GRID_X,
      ...
      this.drawTarget(this.TARGET_X, this.TARGET_Y);

      // Draw the score label
      this.drawScoreLabel(
        this.SCORE_LABEL_X,
        this.SCORE_LABEL_Y,
        this.SCORE_LABEL_WIDTH,
        this.SCORE_LABEL_HEIGHT,
        this.TEXT_SIZE
      );

      // Draw the row selectors
      this.drawRowSelectors(
        this.rowCircleX,
        this.rowCircleY,
        this.CIRCLE_SIZE,
        this.NUM_OF_ROWS,
      ...
      // Draw the column selectors
      this.drawColumnSelectors(
        this.columnCircleX,
        this.columnCircleY,
        this.CIRCLE_SIZE,
        this.NUM_OF_COLS,
    ...
    drawColourWells(x, y, numWells, wellSize, spacing, offset) {
      for (let i = 0; i < numWells; i++) {
        x = offset + (wellSize + spacing) * i;
        let currentColor;
        switch (i) {
          case 0:
            currentColor = this.RED_COLOR;
            break;
          case 1:
            currentColor = this.BLUE_COLOR;
            break;
          case 2:
            currentColor = this.GREEN_COLOR;
            break;
          case 3:
            currentColor = this.YELLOW_COLOR;
            break;
          case 4:
            currentColor = this.ORANGE_COLOR;
            break;
          case 5:
            currentColor = this.PURPLE_COLOR;
            break;
          default:
            currentColor = this.WHITE_COLOR;
            break;
        }

        fill(currentColor);
        stroke(this.GREY_COLOR);
        rect(x, y, wellSize, wellSize);
      }
    }
    ...
    drawTarget(x, y) {
      image(this.loadedTargetImage, x, y);
    }
    ...
    drawScoreLabel(x, y, labelWidth, labelHeight, labelSize) {
      ...
      let scoreLabelY = y + (labelHeight + textAscent()) / 2;
  ...
  window.addEventListener("load", initializePuzzleGame);
}
